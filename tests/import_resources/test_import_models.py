    ...
    def test(self):
        ...
        self.assertEqual(models[0]['description'],
                         ('<div>\n'
                             ...
                             '  <br/></p>\n'
                             '</div>'))
        self.assertEqual(models[0]['taxon'], {
            "id": 7787,
            "name": "Tetronarce californica"
        })
        self.assertEqual(models[0]['tags'], [])
        self.assertEqual(models[0]['identifiers'], [{
            "namespace": "biomodels.db",
            "id": "BIOMD0000000001"
        }])
        self.assertEqual(models[0]['refs'], [{
        ...
        self.assertEqual(models[0]['authors'][0], {
            "lastName": "Edelstein",
            "firstName": "S",
            "middleName": "J"
        })
        self.assertEqual(models[0]['license'], 'CC0')
