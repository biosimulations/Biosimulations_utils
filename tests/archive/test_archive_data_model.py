...
class ArchiveDataModelTestCase(unittest.TestCase):
    ...
    def test_ArchiveFile(self):
        now = datetime.datetime.now()
        file1 = ArchiveFile(filename='./models/model.xml', format=ModelFormat.sbml, description='Description',
                            authors=[Person(first_name='John', last_name='Doe')], created=now, updated=now)
        file2 = ArchiveFile(filename='./models/model.xml', format=ModelFormat.sbml, description='Description',
                            authors=[Person(first_name='John', last_name='Doe')], created=now, updated=now)
        file3 = ArchiveFile(filename='./models/model2.xml', format=ModelFormat.sbml, description='Description',
                            authors=[Person(first_name='John', last_name='Doe')], created=now, updated=now)
