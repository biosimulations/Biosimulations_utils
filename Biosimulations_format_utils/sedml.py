...
def gen_sedml(model_species, sim, model_filename, sim_filename, level=1, version=3):
    ...
    _call_sedml(doc, doc, 'setLevel', level)
    _call_sedml(doc, doc, 'setVersion', version)
    ...
    if sim.get('authors', None):
        notes.append({'label': 'Author(s)', 'value': '<ul>{}</ul>'.format(''.join(
            '<li>{}</li>'.format(saxutils.escape(_format_person_name(author))) for author in sim['authors']))})
    ...
    if sim.get('refs', None):
        notes.append({
            'label': 'References',
            'value': '<ul>{}</ul>'.format(''.join('<li>{}</li>'.format(_format_reference(ref)) for ref in sim['refs'])),
        })
    ...
    if notes:
        ...
        _call_sedml(doc, doc, 'setNotes', notes_xml)

    # model
    model_sedml = doc.createModel()
    _call_sedml(doc, model_sedml, 'setId', 'model')
    _call_sedml(doc, model_sedml, 'setSource', model_filename)
    if sim['model']['format']['name'] == 'SBML':
        _call_sedml(doc, model_sedml, 'setLanguage', 'urn:sedml:sbml')
    else:
    ...
    for model_change in sim['modelParameterChanges']:
        ...
        if sim['model']['format']['name'] == 'SBML':
            _call_sedml(doc, model_change_sedml, 'setTarget',
        ...
        _call_sedml(doc, model_change_sedml, 'setNewValue', str(model_change['value']))

    # simulation
    sim_sedml = doc.createUniformTimeCourse()
    _call_sedml(doc, sim_sedml, 'setId', 'simulation')
    _call_sedml(doc, sim_sedml, 'setInitialTime', sim['startTime'])
    _call_sedml(doc, sim_sedml, 'setOutputStartTime', sim['startTime'])
    _call_sedml(doc, sim_sedml, 'setOutputEndTime', sim['endTime'])
    _call_sedml(doc, sim_sedml, 'setNumberOfPoints', sim['numTimePoints'])

    # simulation algorithm
    alg_sedml = sim_sedml.createAlgorithm()
    _call_sedml(doc, alg_sedml, 'setKisaoID', sim['algorithm']['id'])

    # simulation algorithm parameters
    for param in sim['algorithmParameterChanges']:
        param_sedml = alg_sedml.createAlgorithmParameter()
        _call_sedml(doc, param_sedml, 'setKisaoID', param['parameter']['kisaoId'])
        _call_sedml(doc, param_sedml, 'setValue', str(param['value']))

    # create a task to simulate the model
    task_sedml = doc.createTask()
    _call_sedml(doc, task_sedml, 'setId', 'task')
    _call_sedml(doc, task_sedml, 'setModelReference', model_sedml.getId())
    _call_sedml(doc, task_sedml, 'setSimulationReference', sim_sedml.getId())

    # create a report for the simulation predictions
    report_sedml = doc.createReport()
    _call_sedml(doc, report_sedml, 'setId', 'report')
    _call_sedml(doc, report_sedml, 'setName', 'report')

    # add the simulation time to the report
    data_gen_sedml = doc.createDataGenerator()
    _call_sedml(doc, data_gen_sedml, 'setId', 'data_generator_time')
    _call_sedml(doc, data_gen_sedml, 'setName', 'time')

    var_sedml = data_gen_sedml.createVariable()
    _call_sedml(doc, var_sedml, 'setId', 'var_time')
    _call_sedml(doc, var_sedml, 'setName', 'time')
    _call_sedml(doc, var_sedml, 'setTaskReference', 'task')
    _call_sedml(doc, var_sedml, 'setSymbol', 'urn:sedml:symbol:time')
    _call_sedml(doc, data_gen_sedml, 'setMath', libsedml.parseFormula(var_sedml.getId()))

    dataset_sedml = report_sedml.createDataSet()
    _call_sedml(doc, dataset_sedml, 'setId', 'dataset_time')
    _call_sedml(doc, dataset_sedml, 'setLabel', 'time')
    _call_sedml(doc, dataset_sedml, 'setDataReference', data_gen_sedml.getId())

    # add the simulation predictions to the report
    for species in model_species:
        data_gen_sedml = doc.createDataGenerator()
        _call_sedml(doc, data_gen_sedml, 'setId', 'data_generator_{}'.format(species['id']))
        _call_sedml(doc, data_gen_sedml, 'setName', species['id'])

        var_sedml = data_gen_sedml.createVariable()
        _call_sedml(doc, var_sedml, 'setId', 'var_{}'.format(species['id']))
        _call_sedml(doc, var_sedml, 'setName', species['id'])
        _call_sedml(doc, var_sedml, 'setTaskReference', 'task')
        if sim['model']['format']['name'] == 'SBML':
            _call_sedml(doc, var_sedml, 'setTarget',
        ...
        dataset_sedml = report_sedml.createDataSet()
        _call_sedml(doc, dataset_sedml, 'setId', 'dataset_{}'.format(species['id']))
        _call_sedml(doc, dataset_sedml, 'setLabel', species['id'])
        _call_sedml(doc, dataset_sedml, 'setDataReference', data_gen_sedml.getId())
...
def _call_sedml(doc, obj, method_name, *args, **kwargs):
    ...
    if return_val != 0:
        ...
        for i_error in range(doc.getNumErrors()):
            msgs.append(doc.getError(i_error))
